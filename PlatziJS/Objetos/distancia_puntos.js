//Esta funcion la utilizo para crear puntos en forma de objeto
//este bloque se llama constructor
function Punto(x, y){
  this.x = x
  this.y = y
  //No hace falta poner return porque js ya lo hace
}

//Los metodos para la clase se definen de esta forma.
//Afectara a todos los objetos
Punto.prototype.moverEnX = function moverEnX(x) { this.x += x }
Punto.prototype.moverEnY = function moverEnY(y) { this.y += y }
Punto.prototype.distancia = function distancia(p) {
  const x = this.x - p.x
  const y = this.y - p.y

  return Math.sqrt(x * x + y * y)
}


//Así se crean los objetos con el prototipo de la función
const p2 = new Punto(3, 0)
const p1 = new Punto(0, 4)



console.log(p1.distancia(p2));
console.log(p2.distancia(p1))
p1.moverEnX(10);
console.log(p1.distancia(p2));
p2.moverEnY(-4);
console.log(p1.distancia(p2));

/* Los objetos están basados en prototipos, cada cambio en el prototipo creará cambios en el resto de los objetos
